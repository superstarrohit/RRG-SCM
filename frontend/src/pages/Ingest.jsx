...
import { Panel, PageHeader, DataTable, useApi, fmtNum } from "../components/ui.jsx";
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Upload daily dumps from Excel/CSV, or pull directly from SQL Server, MySQL, PostgreSQL, MS Access or ODBC."
      />

      <div className="seg" style={{ marginBottom: 18 }}>
        <button className={tab === "file" ? "on" : ""} onClick={() => setTab("file")}>📄 File Upload</button>
        <button className={tab === "db" ? "on" : ""} onClick={() => setTab("db")}>🗄 Database Connection</button>
      </div>
      ...
      <Panel title="Recent Ingestions">
        ...
        <DataTable
          columns={[
            ...
            { key: "dump_type", label: "Dump", render: (v) => <span className="mono">{v}</span> },
            { key: "source_type", label: "Source" },
            { key: "source_name", label: "Name", render: (v) => <span className="mono">{v}</span> },
            { key: "rows_ingested", label: "Rows", num: true, render: (v) => fmtNum(v) },
            { key: "mode", label: "Mode" },
            { key: "status", label: "Status", render: (v) => <span className={`badge ${v === "success" ? "b-green" : "b-red"}`}>{v}</span> },
          ]}
          rows={log.data || []}
        />
      </Panel>
    ...
    <div className="alert success">
      ...
      {r.column_map && (
        <div style={{ marginTop: 6 }}>
          {Object.entries(r.column_map).map(([src, dst]) => (
            <span className="tag" key={src}>{src} → {dst}</span>
          ))}
        </div>
      )}
    </div>
...
function PreviewBox({ preview }) {
  ...
  return (
    <div className={`alert ${missing.length ? "error" : "info"}`} style={{ marginTop: 10 }}>
      <div><b>{fmtNum(preview.rows)}</b> rows · {(preview.source_columns || []).length} columns.</div>
      {preview.column_map && (
        <div style={{ marginTop: 6 }}>
      ...
      {missing.length > 0 && (
        <div style={{ marginTop: 6 }}>⚠ Missing required fields: <b>{missing.join(", ")}</b>. Rename or map those columns.</div>
      )}
    </div>
  );
}
...
function FileUpload({ dumps, onDone }) {
  ...
  const doPreview = async () => {
    ...
    try { setPreview(await api.previewFile(build())); }
  ...
  const doUpload = async () => {
    ...
    try { setResult(await api.uploadFile(build())); setPreview(null); onDone(); }
    catch (e) { setResult({ error: e.message }); }
    finally { setBusy(false); }
  };
  ...
  return (
    <Panel title="Upload Excel / CSV">
      <div className="form-grid">
        <div className="field">
          ...
          <select value={dumpType} onChange={(e) => setDumpType(e.target.value)}>
            {dumps.map((d) => <option key={d.key} value={d.key}>{d.label}</option>)}
          </select>
        </div>
        <div className="field">
      ...
      <div className="controls">
        <button className="btn ghost" onClick={doPreview} disabled={!file || busy}>Preview mapping</button>
        <button className="btn blue" onClick={doUpload} disabled={!file || busy}>{busy ? "Working…" : "Upload & Load"}</button>
      </div>
      <PreviewBox preview={preview} />
      <div style={{ marginTop: 10 }}><Result result={result} /></div>
...
function DbConnect({ dumps, sources, onDone }) {
  const [cfg, setCfg] = React.useState({
    source_type: "postgres", host: "", port: "", database: "", username: "", password: "",
    odbc_driver: "", dsn: "", file_path: "", table: "", query: "", dump_type: "", mode: "replace", limit: 100,
  });
  ...
  const st = cfg.source_type, isAccess = st === "access", isOdbc = st === "odbc";
    ...
    <Panel title="Connect to a Database">
      <div className="form-grid">
        ...
        {isAccess ? (
          ...
          <>
            <div className="field"><label>Host</label><input value={cfg.host} onChange={(e) => set("host", e.target.value)} placeholder="localhost" /></div>
            <div className="field"><label>Port</label><input value={cfg.port} onChange={(e) => set("port", e.target.value)} placeholder="(default)" /></div>
            <div className="field"><label>Database</label><input value={cfg.database} onChange={(e) => set("database", e.target.value)} /></div>
          </>
        )}

        {!isAccess && (
          <>
            <div className="field"><label>Username</label><input value={cfg.username} onChange={(e) => set("username", e.target.value)} /></div>
            <div className="field"><label>Password</label><input type="password" value={cfg.password} onChange={(e) => set("password", e.target.value)} /></div>
          </>
        )}

        {(st === "sqlserver" || isOdbc) && (
          <div className="field"><label>ODBC driver</label><input value={cfg.odbc_driver} onChange={(e) => set("odbc_driver", e.target.value)} placeholder="ODBC Driver 17 for SQL Server" /></div>
        )}

        <div className="field"><label>Table (or use a query below)</label><input value={cfg.table} onChange={(e) => set("table", e.target.value)} placeholder="dbo.open_pos" /></div>
        <div className="field">
          <label>Load mode</label>
      ...
      <div className="controls">
        <button className="btn ghost" onClick={doTest} disabled={busy}>Test connection</button>
        <button className="btn ghost" onClick={doPreview} disabled={busy}>Preview</button>
        <button className="btn blue" onClick={doIngest} disabled={busy}>{busy ? "Working…" : "Pull & Load"}</button>
      </div>

      {msg && <div className={`alert ${msg.type}`} style={{ marginTop: 10 }}>{msg.text}</div>}
      {preview && (
        <div className="alert info" style={{ marginTop: 10 }}>
          <b>{fmtNum(preview.rows)}</b> rows · columns: {(preview.source_columns || []).map((c) => <span className="tag" key={c}>{c}</span>)}
        </div>
      )}
      <div style={{ marginTop: 10 }}><Result result={result} /></div>
      ...
      <div className="alert info" style={{ marginTop: 12 }}>
        ...
        <span className="tag">pip install psycopg2-binary</span>
        <span className="tag">PyMySQL</span>
        <span className="tag">pyodbc</span>
      </div>
    </Panel>
  );
}
...
function FieldHints({ dumps, dumpType }) {
  ...
  return (
    <div className="alert info" style={{ marginTop: 12 }}>
      ...
      <div style={{ marginTop: 6 }}>
        {dump.fields.map((f) => <span className="tag" key={f.name}>{f.name}{f.required ? " *" : ""}</span>)}
      </div>
      <div className="muted" style={{ marginTop: 4 }}>* required · columns are matched flexibly (aliases, case, spacing).</div>
    </div>
  );
}
